...
public class ScheduleController {
    ...
    public Mono<ResponseEntity<Integer>> saveSchedule(
        ...
        if (adminKey == null || !adminSecretKey.equals(adminKey)) {
            return Mono.error(new RuntimeException("Access denied: Invalid or missing Admin Key"));
        }

        return scheduleService.addSchedule(scheduleDto).map(id -> ResponseEntity.status(HttpStatus.CREATED).body(id));
    }
